...
if [ "$VERIFY_ONLY" = "0" ]; then
	...
	#
	# THE ARTEFACT MANIFEST. Every evidence script used to carry its own
	# hard-coded EXPECT_VERSION, frozen at whatever build was current when it
	# was written -- mbhc-switch-rc6, rx-dai-rc1, regcache-rc9, core-init-rc2
	# in the lib itself. They failed loudly the day the version moved on,
	# which was luck: a stale default that still MATCHES quietly validates the
	# wrong artefact, and every milestone after it inherits the doubt.
	#
	# The build is the only thing that actually knows what it produced, so it
	# writes that down here and the scripts read it. Copy this file to the
	# phone beside the tools and every run picks up the right expectation with
	# no arguments at all.
	#
	MANIFEST="$MODULE_DIR/wcd9320-artifact.manifest"
	{
		echo "# wcd9320 artefact manifest -- generated by build-wcd9320-kernel.sh"
		echo "# Do not hand-edit: it describes a build, not an intention."
		echo "version=$VERSION"
		echo "sha256=$(sha256sum "$MODULE_DIR/wcd9320.ko" | cut -d' ' -f1)"
		echo "size=$(stat -c%s "$MODULE_DIR/wcd9320.ko")"
		echo "pkgrel=$PKGREL"
		echo "kernel=$KVER"
